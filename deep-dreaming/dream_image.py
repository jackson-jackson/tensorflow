...
and num_repeats really varies on which layer you're using.
...
print('\nStarting...')
print(f"{start_time.strftime('%I:%M:%S %p')}\n")


layer = 3
layer_tensor = model.layer_tensors[layer]
file_name = "inputs/the-starry-night-800x450.jpg"
...
img_result = recursive_optimize(layer_tensor=layer_tensor, image=img_result,
                 ...
                 num_iterations=num_iterations, step_size=1.0, rescale_factor=0.5,
...
result.save(f"outputs/dream_image_out-{layer}-{num_iterations}.jpg")
result.show()


print('\n\nFinished...')
finish_time = datetime.datetime.now()
print(f"{finish_time.strftime('%I:%M:%S %p')}")
total_time = finish_time - start_time
seconds = total_time.total_seconds()
hours = int(seconds / 3600)
minutes = int(seconds / 60) % 60
print(f"Elapsed time: {hours}hrs {minutes}mins {round(seconds, 2)}secs\n")